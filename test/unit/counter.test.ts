import { Readable } from 'stream';
import { getMetricsFromStream, PromData } from '../../src/prom'

it('parses a simple counter', () => {
    const input = Readable.from([`
# HELP go_memstats_alloc_bytes_total Total number of bytes allocated, even if freed.
# TYPE go_memstats_alloc_bytes_total counter
go_memstats_alloc_bytes_total 3.7156890216e+10    
    `]);

    return getMetricsFromStream(input).then(metrics => {
        expect(Object.keys(metrics).length).toEqual(1);

        const metric = metrics['go_memstats_alloc_bytes_total'];

        expect(metric).not.toBeNull();
        expect(metric.type).toEqual('counter');
        expect(metric.data).not.toBeNull();
        expect(metric.data.length).toEqual(1);

        const datum = metric.data[0] as PromData;

        expect(datum.value).toEqual(3.7156890216e+10);
    });
});

it('parses a counter with labels', () => {
    const input = Readable.from([`
# TYPE foo counter
foo{bar="a"} 100
foo{bar="b"} 200
foo{bar="c"} 300 1003
    `]);

    return getMetricsFromStream(input).then(metrics => {
        expect(Object.keys(metrics).length).toEqual(1);

        const metric = metrics['foo'];

        expect(metric).not.toBeNull();
        expect(metric.type).toEqual('counter');
        expect(metric.help).toEqual('');
        expect(metric.data).not.toBeNull();
        expect(metric.data.length).toEqual(3);

        let data: PromData[];

        data = (metric.data as PromData[]).filter(datum => datum.labels['bar'] === 'a');

        expect(data.length).toEqual(1);
        expect(data[0].value).toEqual(100);
        expect(data[0].timestamp).toBeUndefined();

        data = (metric.data as PromData[]).filter(datum => datum.labels['bar'] === 'b');

        expect(data.length).toEqual(1);
        expect(data[0].value).toEqual(200);
        expect(data[0].timestamp).toBeUndefined();

        data = (metric.data as PromData[]).filter(datum => datum.labels['bar'] === 'c');

        expect(data.length).toEqual(1);
        expect(data[0].value).toEqual(300);
        expect(data[0].timestamp).toEqual(1003);
    });
});
